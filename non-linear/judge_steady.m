...
% feedback Gの受動性 == 受動送電条件(ⅰ),(ⅲ)を満たす: (ⅰ)[A is steady] and (ⅲ)[Lo is symmetric positive semi-definite]
% non-linear: 受動送電条件(ⅰ)(ⅲ)を満たす == 蓄積関数 W_G が半正定関数
% non-linear: W_G をリアプノフ関数と見て、システムが周波数安定 == W_G が単調減少
...
% 受動送電条件(ⅲ): Loが対称正定値行列かどうかを判定（semi-definiteより厳しい条件）
Lo = L - C*(A\B);
